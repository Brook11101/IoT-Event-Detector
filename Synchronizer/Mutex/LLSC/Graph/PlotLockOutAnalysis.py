...
def read_execution_times(base_path, group_number):
    """
    从 time_lockout_group_{group_number}.txt 中读取所有的执行时间，
    返回一个一维列表，包含该 group 所有轮次的全部线程耗时（展开）。
    """
    filename = os.path.join(base_path, f"time_lockout_group_{group_number}.txt")
    all_times = []
...
def plot_box_lockout():
    """
    读取 5 个 time_lockout_group_X.txt 文件，绘制箱线图：
      - 统一颜色的箱体
      - 显示中位数(红色)、均值(蓝色虚线)、异常值(红圆点)
      - 添加图例说明
    """
    ...
    plt.title("Execution Time Distribution Without Mutex (LockOut)", fontsize=14)

    # 动态设置 y 轴刻度
    max_val = max(max(g_times) for g_times in data_for_5_groups)
    step = 0.2
    y_ticks = np.arange(0.8, max_val+0.3 + step, step)
    plt.yticks(y_ticks)
