...
class XiaomiCloudConnector:
    ...
    def create_order(self, country):
        url = self.get_api_url(country) + "/scene/edit"
        params = {
            'data': json.dumps(
                {'identify': 'prop.2.1.true.false', 'name': '灯泡-定时', 'st_id': 8,
                 'setting': {'enable_push': '1', 'enable_timer': '1', 'enable_timer_off': '0', 'enable_timer_on': '1',
                             'off_method': 'set_properties',
                             'off_param': [{'value': False, 'piid': 1, 'siid': 2, 'did': '134440830'}],
                             'on_method': 'set_properties',
                             'on_param': [{'value': True, 'piid': 1, 'siid': 2, 'did': '134440830'}],
                             'on_time': '2 10 16 9 1 * 2025', 'off_time': '2 10 16 9 1 * 2025', 'timer_type': '0',
                             'on_filter': '', 'off_filter': ''}, 'authed': ['134440830']}
...
if logged:
    ...
    for current_server in servers:
        result = connector.create_order(current_server)
