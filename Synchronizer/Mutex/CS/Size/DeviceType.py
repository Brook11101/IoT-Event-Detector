...
import redis
import threading
from RuleSet import get_all_rules, DeviceType
...
def init_lock():
    ...
    client.flushall()
    print("Redis 已清空")
    return {device_type: RedisMutexLock(client, device_type) for device_type in DeviceType}


def apply_lock(rule, device_type_lock_dict, time_list):
    """
    执行规则：
    - 按顺序申请锁
    - 记录等待时间
    ...
    - 释放锁
    """
    locks_to_acquire = sorted(rule["DeviceType"])
...
def add_lock_count_to_rules(rules):
    """为每条规则添加独立临界区锁的数量"""
    for rule in rules:
        rule["lock_count"] = len(set(rule["DeviceType"]))
    return rules
...
def execute_rules_for_groups(rule_groups, rounds, base_output_dir):
    ...
    执行规则组，并统计平均等待时间
    """
    device_type_lock_dict = init_lock()

    for group_idx, rules in enumerate(rule_groups):
        unique_locks = {lock for rule in rules for lock in rule["DeviceType"]}
        unique_locks_len = len(unique_locks)
        output_file = f"{base_output_dir}/device_type_lock_groups_{lock_targets[group_idx]}.txt"

        with open(output_file, "w") as file:
            for round_num in range(1, rounds + 1):
                time_list = []
                threads = [threading.Thread(target=apply_lock, args=(rule, device_type_lock_dict, time_list)) for rule in rules]
...
if __name__ == "__main__":
    ...
    rounds = 10
    output_base_dir = r"E:\研究生信息收集\论文材料\IoT-Event-Detector\Synchronizer\Mutex\CS\Size\Data\DeviceType"
