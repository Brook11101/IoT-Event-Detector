...
def create_myisam_table():
    try:
        ...
        if connection is not None:
            cursor = connection.cursor()
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS rule_execution_cv_log_myisam (
            ...
            """)
            print("Table `rule_execution_cv_log_myisam` created with MyISAM engine.")
            connection.close()
    except Error as e:
...
# 插入规则日志（使用 MyISAM）
def insert_log(ruleid, id, timestamp):
    """
    ...
    try:
        ...
        if conflict_rule_ids:
            ...
            cursor.execute(f"""
                SELECT 1 FROM rule_execution_cv_log_myisam
...
def clear_table():
    try:
        ...
        if connection:
            cursor = connection.cursor()
            cursor.execute("TRUNCATE TABLE rule_execution_cv_log_myisam")
            print("Table `rule_execution_cv_log_myisam` cleared successfully.")
            connection.close()
    except Error as e:
