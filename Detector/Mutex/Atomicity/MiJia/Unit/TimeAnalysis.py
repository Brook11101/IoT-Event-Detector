...
for prefix in ["device_name", "device_type", "room"]:
    ...
    plt.plot(rule_counts, all_data[prefix], color=style["color"], linestyle=style["linestyle"], marker="o", label=style["label"])

# 为 home 单独设置右侧刻度轴
ax = plt.gca()
ax2 = ax.twinx()
style = types_styles["home"]
ax2.plot(rule_counts, all_data["home"], color=style["color"], linestyle=style["linestyle"], marker="o", label=style["label"])
...
# 保存和显示
output_image_path = os.path.join(base_path, "lock_time_analysis_with_home.png")
