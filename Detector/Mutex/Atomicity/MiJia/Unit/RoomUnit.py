...
# 模拟规则执行的线程函数
def apply_lock(rule, room_lock_dict, time_differences):
    # Step 1: 获取需要申请的锁并排序
    ...
    with open(output_file, "w") as file:
        for round_num in range(1, rounds + 1):
            ...
            for rule in labeled_rules:
                thread = threading.Thread(target=apply_lock, args=(rule, room_lock_dict, time_differences))
...
if __name__ == "__main__":
    rounds = 20
    output_path = r"E:\\研究生信息收集\\论文材料\\IoT-Event-Detector\\Detector\\Mutex\\Atomicity\\MiJia\\Unit\\Data\\room_lock_"+str(rounds)+".txt"
