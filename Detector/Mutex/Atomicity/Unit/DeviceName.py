...
from RuleSet import DeviceName
...
def add_lock_count_to_rules(rules):
    for rule in rules:
        unique_locks = set(rule["DeviceName"])
...
def group_rules_by_lock_count(rules, lock_targets):
    ...
    for target in lock_targets:
        ...
        total_locks = sum(len(rule["DeviceName"]) for rule in current_group)  # 初始化总锁数量

        while total_locks < target and remaining_rules:
            # 随机选择一个规则
            rule = random.choice(remaining_rules)
            current_group.append(rule)
            total_locks += len(rule["DeviceName"])  # 累加锁数量
...
def execute_rules_for_groups(connector, rule_groups, rounds, base_output_dir):
    ...
    for group_idx, rules in enumerate(rule_groups):
        ...
        output_file = f"{base_output_dir}/device_name_lock_groups_{lock_targets[group_idx]}.txt"
...
if __name__ == "__main__":
    ...
    lock_targets = [5,10,15,20,25,30,35,40,45,50]
    rule_groups = group_rules_by_lock_count(labeled_rules, lock_targets)

    rounds = 10
    output_base_dir = r"E:\研究生信息收集\论文材料\IoT-Event-Detector\Detector\Mutex\Atomicity\Unit\Data\DeviceName"
    username = "2844532281"
    password = "whd123456"
